...
- Interpolateur `sql` pour les requêtes ; fragments `fr` pour la composition dynamique de SQL.
- Dérivez automatiquement les instances `Read` / `Write` ; définissez les `Meta` personnalisés pour les types de domaine.
...
- Définissez le comportement comme une fonction pure retournant `Behavior[T]` ; effets secondaires uniquement dans `setup` ou les gestionnaires de messages.
- Akka Streams pour les pipelines avec contre-pression ; `Source`, `Flow`, `Sink` avec matérialisation explicite.
- Cluster Sharding pour les entités distribuées avec état ; persistance via Event Sourcing.

### Apache Spark
- Utilisez l'API Dataset avec les encodeurs de classe de cas — évitez les RDDs dans le nouveau code.
- Diffusez explicitement les petites tables de recherche ; évitez les jointures lourdes en shuffle sur les grands ensembles de données.
- Partitionnez par la colonne la plus fréquemment utilisée dans les filtres/jointures.
- Tests unitaires de transformations avec `spark-testing-base` ou tests typés `Frameless`.

### SBT
- Builds multi-projets avec un `build.sbt` racine ; paramètres partagés dans `project/Settings.scala`.
- `ThisBuild / scalaVersion` pour définir la version une seule fois.
...
**Entrée :** « Créez un endpoint http4s qui valide une charge JSON entrante, écrit un enregistrement dans PostgreSQL via Doobie, et retourne l'entité créée — tout dans Cats Effect IO. »

**Sortie :** Un `UserRoutes` `HttpRoutes[IO]` avec circe `EntityDecoder`, un `UserRepository` avec un insert Doobie `sql` retournant l'ID généré, `Transactor` géré par `Resource`, une enum `UserError` de domaine scellée mappée aux réponses HTTP, et des tests `munit-cats-effect` utilisant `TestControl`.
