...
Dwingt af dat een plan wordt gepresenteerd, geschreven naar `.claude/plan.md` en goedgekeurd door de gebruiker voordat het Claude is toegestaan om terminalopdrachten uit te voeren of bestanden te bewerken.

## Wanneer deze wordt geactiveerd

Gebeurtenis: `PreToolUse`
...
## Hoe te gebruiken
1. Wanneer een nieuwe taak wordt ontvangen, wordt Claude Code geblokkeerd voor het uitvoeren van acties.
2. Claude schrijft een voorgesteld plan naar `.claude/plan.md` met `Status: Pending`.
3. De gebruiker bewerkt `.claude/plan.md` en vervangt de status door `Status: Approved`.
4. Claudes volgende acties zullen de beveiliging omzeilen en met succes worden uitgevoerd.
