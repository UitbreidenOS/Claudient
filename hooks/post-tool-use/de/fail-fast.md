# Hook: Fail-Fast-Enforcer (Sofortiger Stopp)

Erzwingt einen harten Stopp und blockiert die Ausführung von Claude, wenn ein Terminal-/Bash-Befehl fehlschlägt. Dies verhindert, dass Claude Fehler verschluckt oder bei Fehlern Mock-Daten generiert.
...
Claude Code neigt manchmal dazu, Fehler bei der Kompilierung, bei Tests oder API-Aufrufen zu umgehen, indem es einen Erfolg simuliert oder Mock-Dateien schreibt. Der Fail-Fast-Enforcer blockiert den Tool-Aufruf, stoppt Claudes Ausführung und zeigt dem Entwickler den Fehler sofort an.
