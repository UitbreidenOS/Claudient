---
name: office-docs
description: "Générer des présentations PPTX, des rapports DOCX et des feuilles de calcul XLSX à partir de données structurées — en utilisant python-pptx, python-docx et openpyxl sans nécessiter Microsoft Office"
---

# Office Docs Skill

## Quand activer
- Générer une présentation PowerPoint à partir de contenu ou de données structurées
- Créer un rapport Word à partir d'un modèle ou d'une source de données
- Construire une feuille de calcul Excel avec des données formatées, des formules et des graphiques
- Automatiser la génération de documents dans un pipeline CI/CD ou de données
- Convertir du markdown ou des données structurées en documents Office professionnels

## Quand ne PAS utiliser
- Génération simple de PDF — utiliser une bibliothèque PDF (WeasyPrint, reportlab)
- Google Slides/Docs — utiliser le MCP Google Workspace
- Tableaux de bord interactifs — utiliser un outil BI (Metabase, Grafana)
- Édition collaborative en temps réel — utiliser Google Workspace ou Office 365 directement
...
Générer une présentation PPTX pour [topic].

Topic: [décrire]
Slides: [lister les titres des diapositives et les points clés]
...
Générer un rapport DOCX pour [type].

Type: [technical report / executive summary / proposal / documentation]
Sections: [lister les sections et le contenu]
...
### Feuille de calcul Excel (openpyxl)

```python
Install: pip install openpyxl

Générer une feuille de calcul XLSX pour [purpose].

Purpose: [financial model / data export / dashboard / report]
Sheets: [lister les feuilles et les données]
...
**User:** Je dois générer un rapport mensuel du conseil comme DOCX à partir de nos données de métriques.
