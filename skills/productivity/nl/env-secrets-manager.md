...
# Env & Secrets Manager Vaardigheid

## Wanneer activeren
- Audit van een codebase voor onopzettelijk gecommitteerde geheimen vóór open-sourcing
- Validatie dat .env.example compleet en up-to-date is
- Controleren op git-historiek op geheimen lekkages (voordat ze openbaar zijn)
- Een nieuw project instellen met juiste .env-hygiëne vanaf het begin
- Inwerking van een nieuwe ontwikkelaar die lokale geheimen veilig moet instellen

## Wanneer NIET gebruiken
- Beheer van productiegeheimen — gebruik de secrets-management vaardigheid (Vault, AWS Secrets Manager)
- Geheimen rotatie — gebruik de secrets-management vaardigheid
- CI/CD geheimen injectie — gebruik de cicd vaardigheid
...
### Gelekkagedetectie van geheimen in git-geschiedenis
...
**Gebruiker:** We hebben zojuist een repository open-sourced. Hoe controleer ik of we onopzettelijk geheimen hebben gecommitteerd?

**Claude's auditplan:**

Voer deze 5 commando's uit — ze dekken 90% van de gebruikelijke geheimen:
...
**Als u iets vindt:**
1. Draai de sleutel onmiddellijk — wacht niet
2. GitHub heeft uw repository al gescand (ze waarschuwen leveranciers zoals Stripe, AWS op push)
3. Gaan ervan uit dat de sleutel is aangetast als de repository op enig moment openbaar was
4. Gebruik git-filter-repo om het uit de geschiedenis te verwijderen + force push
