...
## Wanneer activeren
- Een edge- of serverless-applicatie bouwen die een database nodig heeft
- SQLite instellen voor een Cloudflare Workers- of Bun-gebaseerd project
- Per-tenant databases implementeren (één SQLite per klant)
- Migreren van Planetscale of soortgelijk en edge-compatibele SQL nodig hebben
- Bouwen met de T3 stack of soortgelijk en een gratis, snelle database willen

## Wanneer NIET gebruiken
- Complexe relationele gegevens die joins over veel tabellen vereisen — overweeg PostgreSQL
- Toepassingen met hoge schrijffrequentie (SQLite heeft write-serialisatie)
- Toepassingen die volledige PostgreSQL-extensies nodig hebben (pgvector, enz.) — gebruik Neon
...
**Gebruiker:** Ik wil een persoonlijk project met een database bouwen. Ik wil niet voor hosting betalen. Wat is het eenvoudigste pad?

**Claude's aanbeveling:**

**Turso-gratis tier is onverslaanbaar voor persoonlijke projecten:**
- 9GB opslag gratis
- 500 databases gratis (één per project of per tenant)
- Edge-locaties wereldwijd (gegevens dicht bij uw gebruikers)
