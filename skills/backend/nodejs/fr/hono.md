...
## When to activate
- Construction d'une nouvelle API avec Hono sur Cloudflare Workers, Bun ou Node.js
- Migration d'une API Express vers Hono pour le déploiement edge
- Configuration de la validation Zod avec le middleware validateur de Hono
- Utilisation de Hono RPC pour une communication client/serveur type-safe
- Implémentation de middleware (auth, CORS, rate limiting) dans Hono

## When NOT to use
- Routes API Next.js — utilisez la compétence nextjs
- Backend complet avec intégration ORM lourde — Hono fonctionne mais Express/Fastify ont plus d'écosystème
- Grand monolithe avec chaînes de middleware complexes — envisagez NestJS
