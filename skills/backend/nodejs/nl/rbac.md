...
## Wanneer activeren
- Een multi-tenant SaaS bouwen waar gebruikers bij organisaties behoren
- Role-based access control implementeren (admin, editor, viewer, enz.)
- Database-query's scopen zodat gebruikers alleen de gegevens van hun organisatie zien
- Permission middleware toevoegen aan API routes
- Het database schema voor roles en permissions ontwerpen

## Wanneer NIET gebruiken
- Single-tenant apps waar alle geverifieerde gebruikers dezelfde toegang hebben
- Eenvoudige booleaanse `isAdmin` checks — alleen de moeite waard bij 3+ rollen
- Wanneer Better Auth's ingebouwde organisatie plugin uw behoeften dekken (controleer eerst)
...
### De rol van de huidige gebruiker in een organisatie krijgen
...
### Next.js — Server Action met RBAC
...
**Gebruiker:** Een SaaS projectbeheertool — eigenaren kunnen facturering beheren en de org verwijderen, admins kunnen leden uitnodigen en projecten verwijderen, editors kunnen projecten maken, viewers kunnen alleen lezen. Implementeer het schema, permission helper en middleware voor Next.js Server Actions.

**Verwachte output:**
- `db/schema.ts` — organizations, users, memberships (met role enum), projects tables
