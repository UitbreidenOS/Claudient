...
## Quand activer
- Écrire ou réviser des scripts PowerShell pour l'administration Windows ou l'automatisation multiplateforme
- Utiliser des cmdlets, des pipelines et la manipulation d'objets
- Implémenter une gestion robuste des erreurs avec try/catch/finally
- Exécuter des commandes sur des machines distantes avec Invoke-Command ou Enter-PSSession
- Structurer des modules réutilisables (`.psm1` + `.psd1`)
- Interroger les informations système avec CIM/WMI
- Gérer les credentials et les secrets de manière sécurisée

## Quand ne pas utiliser
- Scripting Bash/zsh sur Linux/macOS où PowerShell n'est pas installé
- Scripts PowerShell 5.1 uniquement Windows quand la compatibilité multiplateforme n'est explicitement pas requise et la question concerne les fonctionnalités Windows uniquement (utilisez la compétence windows-admin si disponible)
- Modules Azure PowerShell ou AWS PowerShell — ceux-ci sont couverts par leurs compétences cloud respectives

## Instructions

### Pipeline et Filtrage

Les pipelines PowerShell passent des objets .NET, pas des chaînes de texte. Les cmdlets en aval reçoivent l'objet complet avec toutes ses propriétés.
...
Utilisez `$_` (ou `$PSItem`) dans les blocs de script pour référencer l'objet pipeline actuel.

### Gestion des Erreurs — try/catch/finally et $ErrorActionPreference
...
Variables clés de gestion des erreurs :

| Variable | Objectif |
|---|---|
| `$ErrorActionPreference` | Action par défaut : `Stop`, `Continue`, `SilentlyContinue`, `Inquire` |
| `$_` dans `catch` | L'ErrorRecord attrapé |
| `$_.Exception` | L'exception .NET sous-jacente |
| `$_.InvocationInfo` | Infos de position du script |
| `$Error[0]` | Erreur la plus récente de la session |
| `-ErrorAction` parameter | Remplacement par-cmdlet de `$ErrorActionPreference` |
| `-ErrorVariable` parameter | Capturer les erreurs dans une variable nommée |
...
Activez la communication à distance sur les cibles Windows : `Enable-PSRemoting -Force` (exécuter en tant qu'administrateur).

### Structure du Module — psm1 + psd1
...
Installez à la portée utilisateur : `Install-Module MyModule -Scope CurrentUser`
Publiez sur PSGallery : `Publish-Module -Name MyModule -NuGetApiKey $env:PSGALLERY_KEY`
...
N'emmagasinez jamais les secrets en texte brut dans les scripts. Utilisez `SecretManagement` pour les coffres multiplateforme ou les variables d'environnement injectées par le système CI/CD.

### CIM/WMI pour les Informations Système

Utilisez `Get-CimInstance` (préféré) plutôt que `Get-WmiObject` (déprécié dans PowerShell 7) :
...
### Différences PowerShell 7 Multiplateforme

| Fonctionnalité | Windows PowerShell 5.1 | PowerShell 7 |
|---|---|---|
...
| `Enter-PSSession` SSH | Non supporté | Supporté nativement |
| Ternary operator | Non supporté | `$x ? $a : $b` |
| Null coalescing | Non supporté | `$x ?? $default`, `$x ??= $default` |
| Pipeline chain operators | Non supporté | `command1 && command2`, `command1 \|\| fallback` |
| `ForEach-Object -Parallel` | Non supporté | Supporté (use `-ThrottleLimit`) |
| `Out-GridView` | Disponible | Windows uniquement (requires WinForms) |
...
Un script qui audite l'utilisation du disque sur une ferme de serveurs, collecte les résultats en parallèle et exporte en CSV :
...
Exécutez : `.\disk-audit.ps1 -Servers web01, web02, db01 -ThresholdPercent 80`
