...
## Wanneer activeren
- PowerShell scripts schrijven of reviewen voor Windows administratie of cross-platform automatisering
- Cmdlets, pipelines en object manipulatie gebruiken
- Robust error handling implementeren met try/catch/finally
- Commando's uitvoeren op remote machines met Invoke-Command of Enter-PSSession
- Herbruikbare modules structureren (`.psm1` + `.psd1`)
- Systeminformatie opvragen met CIM/WMI
- Credentials en secrets veilig beheren

## Wanneer niet gebruiken
- Bash/zsh scripting op Linux/macOS waar PowerShell niet geïnstalleerd is
- PowerShell 5.1-only Windows scripts wanneer cross-platform compatibiliteit expliciet niet vereist is en de vraag over Windows-only features gaat (use windows-admin skill als beschikbaar)
- Azure PowerShell of AWS PowerShell modules — deze worden behandeld door hun respectievelijke cloud skills

## Instructies

### Pipeline en Filterering

PowerShell pipelines geven .NET objecten door, geen tekst strings. Downstream cmdlets ontvangen het volledige object met alle eigenschappen.
...
Gebruik `$_` (of `$PSItem`) in script blokken om het huidige pipeline object te refereren.

### Error Handling — try/catch/finally en $ErrorActionPreference
...
Sleutel error-handling variabelen:

| Variabele | Doel |
|---|---|
| `$ErrorActionPreference` | Standaard actie: `Stop`, `Continue`, `SilentlyContinue`, `Inquire` |
| `$_` in `catch` | De gevangen `ErrorRecord` |
| `$_.Exception` | De onderliggende .NET exception |
| `$_.InvocationInfo` | Script positie info |
| `$Error[0]` | Meest recente fout van de sessie |
| `-ErrorAction` parameter | Per-cmdlet override van `$ErrorActionPreference` |
| `-ErrorVariable` parameter | Fouten in een genoemde variabele vastleggen |
...
Enable remoting op Windows targets: `Enable-PSRemoting -Force` (run as Administrator).

### Module Structuur — psm1 + psd1
...
Install in user scope: `Install-Module MyModule -Scope CurrentUser`
...
Sla secrets nooit als plaintext in scripts op. Gebruik `SecretManagement` voor cross-platform vaults of environment variabelen geïnjecteerd door CI/CD systeem.

### CIM/WMI voor Systeminformatie

Gebruik `Get-CimInstance` (preferred) over `Get-WmiObject` (deprecated in PowerShell 7):
...
### PowerShell 7 Cross-Platform Verschillen
...
| `Enter-PSSession` SSH | Not supported | Natively supported |
...
Een script dat schijfgebruik over een serverfarm controleert, resultaten parallel verzamelt en naar CSV exporteert:
