# Agent Team Kickoff Workflow

Initializes a multi-agent team by defining roles, partitioning the task scope, and establishing coordination contracts between agents before execution.

---

## When to use

- Starting a new multi-agent workflow with 3+ agents
- Onboarding new agents into an existing team
- Scaling a workflow to handle larger, more complex tasks
- Ensuring clear role boundaries to prevent agent conflicts
- Establishing communication protocols before agents are unleashed

Do not use for single-agent tasks or workflows where agent roles are already clearly defined and in production.

---

## Role Assignment

Each agent receives a role card defining what it owns, what it depends on, and what it must not do:
...
**Role card rules:**
- One role per agent (agents do not have multiple roles)
- Boundaries are strict — agents should reject work outside their domain
- Dependencies define the sequencing and blocking relationships
- SLA defines the contract for acceptable performance

---

## Scope Partition

Break the overall task into non-overlapping subtasks assigned to specific agents:
...
**Partitioning rules:**
- No task overlap (each task assigned to exactly one agent)
- No gaps (union of all tasks equals the overall goal)
- Explicit dependencies (which subtask must complete before another starts)
- Clear acceptance criteria (how does the next agent know work is done?)

---

## Coordination Contract

Before execution, all agents agree to a coordination contract:
...
- All agents verify contract terms exist before starting work
- Violations (e.g., circular call) trigger immediate escalation
- Supervisor enforces timeout, retry, and resource limits
- Audit log records all contract checks
...
Only proceed to execution if all checklist items are true.

---

## Example

**Research + Analysis + Report workflow kickoff:**
